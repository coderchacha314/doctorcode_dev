...
export async function GET(
  ...
  if (process.env.NEXT_PUBLIC_DEV_DOCTOR_BYPASS === "true") return NextResponse.json({ prescriptions: [] });
...
export async function POST(
  ...
  if (process.env.NEXT_PUBLIC_DEV_DOCTOR_BYPASS === "true") {
    ...
    return NextResponse.json({
      prescription: { id: `rx-${Date.now()}`, ...body, issuedAt: new Date().toISOString(), createdAt: new Date().toISOString() },
    }, { status: 201 });
  }
  const supabase = await createSupabaseServerClient();
  const { data: { user }, error } = await supabase.auth.getUser();
