...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const extracted = await extractFromReport(image, mimeType, `This is a medical lab report or glucometer reading. Extract blood sugar/glucose values if present.
Respond ONLY with a JSON object (use null for missing values):
{
  "value": <number mg/dL or null>,
