...
import { getLipidStatus } from "@/lib/status";
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const [readings, total] = await Promise.all([
    prisma.lipidReading.findMany({ where, orderBy: { recordedAt: "desc" }, take: limit }),
    prisma.lipidReading.count({ where }),
  ]);

  return NextResponse.json({
    readings: readings.map((r) => ({ ...r, status: getLipidStatus(r.ldl) })),
    total,
  });
}
...
const postSchema = z.object({
  ...
  vldl: z.number().positive().optional(),
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const reading = await prisma.lipidReading.create({
    data: { ...data, patientId: patient.id, recordedAt: recordedAt ? new Date(recordedAt) : new Date() },
  });

  return NextResponse.json({ reading: { ...reading, status: getLipidStatus(reading.ldl) } }, { status: 201 });
}
