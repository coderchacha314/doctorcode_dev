...
import { getLiverStatus } from "@/lib/status";
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const [readings, total] = await Promise.all([
    prisma.liverReading.findMany({ where, orderBy: { recordedAt: "desc" }, take: limit }),
    prisma.liverReading.count({ where }),
  ]);

  return NextResponse.json({
    readings: readings.map((r) => ({ ...r, status: getLiverStatus(r.alt) })),
    total,
  });
}
...
const postSchema = z.object({
  ...
  alp: z.number().positive().optional(),
  bilirubin: z.number().positive().optional(),
  albumin: z.number().positive().optional(),
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const reading = await prisma.liverReading.create({
    data: { ...data, patientId: patient.id, recordedAt: recordedAt ? new Date(recordedAt) : new Date() },
  });

  return NextResponse.json({ reading: { ...reading, status: getLiverStatus(reading.alt) } }, { status: 201 });
}
