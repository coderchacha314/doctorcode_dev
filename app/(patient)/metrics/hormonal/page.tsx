...
export default function HormonalPage(): React.ReactElement {
  const { theme } = useTheme();
  const isDark = theme === "dark";
  const [range, setRange] = useState<Range>("7D");
  const [readings, setReadings] = useState<HormonalReading[]>([]);
  ...
  const [formTsh, setFormTsh] = useState("");
  const [formT3, setFormT3] = useState("");
  const [formT4, setFormT4] = useState("");
  const [formCortisol, setFormCortisol] = useState("");
  ...
  const fetchReadings = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const res = await fetch(`/api/readings/hormonal?limit=100&from=${from.toISOString()}`);
  ...
  async function handleScan(e: React.ChangeEvent<HTMLInputElement>): Promise<void> {
    ...
    try {
      ...
      const res = await fetch("/api/readings/hormonal/scan", {
        method: "POST",
      ...
      if (extracted.tsh != null) setFormTsh(String(extracted.tsh));
      if (extracted.t3 != null) setFormT3(String(extracted.t3));
      if (extracted.t4 != null) setFormT4(String(extracted.t4));
      if (extracted.cortisol != null) setFormCortisol(String(extracted.cortisol));
      if (extracted.insulin != null) setFormInsulin(String(extracted.insulin));
  ...
  async function handleSave(e: React.FormEvent): Promise<void> {
    e.preventDefault();
    setSaveError("");
    if (!formTsh && !formT3 && !formT4 && !formCortisol && !formInsulin) { setSaveError("Enter at least one value."); return; }
    if (!formDate || !formTime) { setSaveError("Date and time are required."); return; }
    setSaving(true);
    try {
      const res = await fetch("/api/readings/hormonal", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          tsh: formTsh ? parseFloat(formTsh) : undefined,
          t3: formT3 ? parseFloat(formT3) : undefined,
          t4: formT4 ? parseFloat(formT4) : undefined,
          cortisol: formCortisol ? parseFloat(formCortisol) : undefined,
          insulin: formInsulin ? parseFloat(formInsulin) : undefined,
          recordedAt: new Date(`${formDate}T${formTime}`).toISOString(),
        }),
      });
      if (!res.ok) { const d = await res.json(); setSaveError(d.error ?? "Failed"); return; }
      setFormTsh(""); setFormT3(""); setFormT4(""); setFormCortisol(""); setFormInsulin(""); setFormDate(""); setFormTime("");
      setRefreshKey((k) => k + 1);
    ...
    <div className="flex flex-col">
      <div className="px-4 pt-12 pb-4" style={{ backgroundColor: "var(--color-surface)" }}>
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            <h2 className="text-2xl font-bold" style={{ color: "var(--color-text)" }}>Hormonal</h2>
            {latest && <StatusBadge status={statusToBadge(latest.status)} />}
          </div>
        </div>
        <div className="mt-4 flex gap-6">
          <div>
            <p className="text-xs mb-1" style={{ color: "var(--color-text-muted)" }}>TSH</p>
            {latest?.tsh != null ? (
              <div className="flex items-baseline gap-1">
                <span className="text-4xl font-bold" style={{ color: "var(--color-text)" }}>{latest.tsh.toFixed(2)}</span>
                <span className="text-sm" style={{ color: "var(--color-text-muted)" }}>mIU/L</span>
              </div>
            ) : <span className="text-xl" style={{ color: "var(--color-text-muted)" }}>—</span>}
          </div>
          {latest?.t3 != null && <div>
            <p className="text-xs mb-1" style={{ color: "var(--color-text-muted)" }}>T3</p>
            <div className="flex items-baseline gap-1">
              <span className="text-4xl font-bold" style={{ color: "var(--color-text)" }}>{latest.t3.toFixed(1)}</span>
              <span className="text-sm" style={{ color: "var(--color-text-muted)" }}>ng/dL</span>
            </div>
          </div>}
          {latest?.t4 != null && <div>
            <p className="text-xs mb-1" style={{ color: "var(--color-text-muted)" }}>T4</p>
            <div className="flex items-baseline gap-1">
              <span className="text-4xl font-bold" style={{ color: "var(--color-text)" }}>{latest.t4.toFixed(1)}</span>
              <span className="text-sm" style={{ color: "var(--color-text-muted)" }}>μg/dL</span>
            </div>
          </div>}
        </div>
      </div>
          ...
          <form onSubmit={handleSave} className="px-4 pb-4 flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1">
                <label className="text-xs uppercase tracking-wider" style={{ color: "var(--color-text-muted)" }}>TSH (mIU/L)</label>
                <input type="number" inputMode="decimal" step="0.01" placeholder="2.0" value={formTsh} onChange={(e) => setFormTsh(e.target.value)} className="input-field" />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs uppercase tracking-wider" style={{ color: "var(--color-text-muted)" }}>T3 (ng/dL)</label>
                <input type="number" inputMode="decimal" step="0.1" placeholder="120" value={formT3} onChange={(e) => setFormT3(e.target.value)} className="input-field" />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs uppercase tracking-wider" style={{ color: "var(--color-text-muted)" }}>T4 (μg/dL)</label>
                <input type="number" inputMode="decimal" step="0.1" placeholder="7.5" value={formT4} onChange={(e) => setFormT4(e.target.value)} className="input-field" />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs uppercase tracking-wider" style={{ color: "var(--color-text-muted)" }}>Cortisol (μg/dL)</label>
                <input type="number" inputMode="decimal" step="0.1" placeholder="15" value={formCortisol} onChange={(e) => setFormCortisol(e.target.value)} className="input-field" />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs uppercase tracking-wider" style={{ color: "var(--color-text-muted)" }}>Insulin (μU/mL)</label>
                <input type="number" inputMode="decimal" step="0.1" placeholder="10" value={formInsulin} onChange={(e) => setFormInsulin(e.target.value)} className="input-field" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1">
      ...
      <div className="px-4 mt-4">
        <div className="card p-4">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold" style={{ color: "var(--color-text-2)" }}>TSH Trend</h3>
          ...
          : mounted ? (
            <ResponsiveContainer width="100%" height={160}>
              <LineChart data={chartData}>
                ...
                <ReferenceLine y={0.4} stroke="rgba(34,197,94,0.3)" strokeDasharray="4 4" />
                <ReferenceLine y={4.0} stroke="rgba(34,197,94,0.3)" strokeDasharray="4 4" />
                <Line type="monotone" dataKey="tsh" stroke="#a78bfa" strokeWidth={2.5} dot={{ fill: "#a78bfa", r: 4, strokeWidth: 0 }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          ) : null}
        </div>
            ...
            <div key={entry.id} className="card px-4 py-3 flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs mt-0.5" style={{ color: "var(--color-text-muted)" }}>
                  {[entry.t3 != null && `T3: ${entry.t3}`, entry.t4 != null && `T4: ${entry.t4}`, entry.cortisol != null && `Cortisol: ${entry.cortisol}`].filter(Boolean).join(" · ") || "—"}
                </p>
              </div>
              <div className="flex flex-col items-end gap-1">
                <div className="flex items-baseline gap-1">
                  <span className="text-lg font-bold" style={{ color: "var(--color-text)" }}>{entry.tsh?.toFixed(2) ?? "—"}</span>
                  <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>mIU/L</span>
                </div>
