...
export default function KidneyPage(): React.ReactElement {
  ...
  async function handleScan(e: React.ChangeEvent<HTMLInputElement>): Promise<void> {
    ...
    try {
      ...
      const res = await fetch("/api/readings/kidney/scan", {
        method: "POST",
      ...
      if (extracted.creatinine != null) setFormCreatinine(String(extracted.creatinine));
      if (extracted.bun != null) setFormBun(String(extracted.bun));
      if (extracted.egfr != null) setFormEgfr(String(extracted.egfr));
      if (extracted.uricAcid != null) setFormUricAcid(String(extracted.uricAcid));
      if (extracted.reportDate) {
        setFormDate(extracted.reportDate);
        setFormTime("08:00");
      }
        ...
        <details className="card overflow-hidden" open>
          ...
          <form onSubmit={handleSave} className="px-4 pb-4 flex flex-col gap-3">
            ...
            <button
              ...
              disabled={scanning}
              className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl border border-dashed border-blue-500/40 text-blue-400 text-sm font-medium transition-colors hover:bg-blue-500/10"
            >
